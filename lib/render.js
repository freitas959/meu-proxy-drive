...
/**
 * Selo de verificado ao lado do nome, no layout de post.
 *
 * Desenhado à mão em vez de carregar um arquivo: o canvas exporta o PNG na
 * hora, e uma imagem externa entraria como mais um carregamento assíncrono que
 * pode não ter terminado quando o card é desenhado.
 *
 * A forma é a rosácea de 12 pontas. Ela sai do polígono estrelado com o traço
 * grosso e junta arredondada por cima — é o traço que arredonda as pontas, sem
 * precisar de curva de Bézier ponta a ponta.
 */
function desenharVerificado(ctx, cx, cy, raio, cor = "#2a7bf0") {
  const LOBOS = 10;
  // Lóbulo pequeno e disco central grande: o vale fica em 0,86 do raio, que é a
  // profundidade da referência. Com lóbulo maior a forma vira nuvem.
  const raioLobo = raio * 0.2;
  const distancia = raio - raioLobo;
  const raioDisco = raio * 0.86;

  ctx.save();

  // A rosácea sai da união de um disco central com um círculo em cada lóbulo.
  // Feito com polígono estrelado, os vales ficam em ponta; com círculos, saem
  // com a mesma curvatura das pontas, que é o que a referência tem.
  ctx.fillStyle = cor;
  ctx.beginPath();
  ctx.arc(cx, cy, raioDisco, 0, Math.PI * 2);
  ctx.fill();
  for (let i = 0; i < LOBOS; i++) {
    const ang = ((Math.PI * 2) / LOBOS) * i - Math.PI / 2;
    ctx.beginPath();
    ctx.arc(cx + Math.cos(ang) * distancia, cy + Math.sin(ang) * distancia, raioLobo, 0, Math.PI * 2);
    ctx.fill();
  }

  // Check com ponta reta e canto vivo: na referência ele é um traço chapado,
  // não uma linha arredondada.
  ctx.beginPath();
  ctx.moveTo(cx - raio * 0.40, cy + raio * 0.02);
  ctx.lineTo(cx - raio * 0.11, cy + raio * 0.31);
  ctx.lineTo(cx + raio * 0.42, cy - raio * 0.30);
  ctx.strokeStyle = "#ffffff";
  ctx.lineWidth = raio * 0.26;
  ctx.lineCap = "butt";
  ctx.lineJoin = "miter";
  ctx.stroke();
...
async function layoutTweet(ctx, opcoes) {
  ...
  if (perfil?.verificado) {
    ...
    desenharVerificado(ctx, xTexto + larguraNome + 20, topo + 20, 11);
  }
