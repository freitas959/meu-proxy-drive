...
async function layoutEditorial(ctx, opcoes) {
  const { W, H, card, template, fontes, indice, total, imagem, handle, ehCapa } = opcoes;
  const paleta = paletaDoCard(opcoes.paleta, template, indice, ehCapa);
...
async function layoutChat(ctx, opcoes) {
  const { W, H, card, template, fontes, indice, handle, ehCapa } = opcoes;
...
async function layoutTweet(ctx, opcoes) {
  const { W, H, card, paleta, fontes, indice, total, handle, perfil } = opcoes;
  const pad = Math.round(W * 0.07);
  const larguraUtil = W - pad * 2;
