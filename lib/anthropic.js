...
/**
 * Normaliza um campo que o schema declara como lista.
 *
 * Mesmo com `type: "array"` declarado, o modelo às vezes entrega a lista
 * serializada como texto. O perigo é silencioso: `"[{...}]".length` é
 * verdadeiro e `.slice()` funciona numa string, então validar por `.length`
 * deixa a string passar inteira pelo servidor — e ela só estoura lá na frente,
 * no `.map()` do navegador, com "slides.map is not a function".
 *
 * Devolve `null` quando não dá pra chegar num array, para quem chama poder
 * estornar o crédito e responder um erro de verdade.
 */
